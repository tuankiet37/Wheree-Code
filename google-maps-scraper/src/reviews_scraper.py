...
import pandas as pd
from utils import prepare_file, writeMainInfo, writeFile
import os
...
def scrape_reviews_for_url(row, output_file):
    n_reviews = 100
    hl = "en"
    sort_by = "qualityScore"
    with GoogleMapsAPIScraper() as scraper:
        data = scraper.scrape_reviews(url=row['map_url'], n_reviews=n_reviews, hl=hl, sort_by=sort_by)
        results = []
        for review_data in data:
            results_entry = {
                'code': row['code'],  
                'reviews': review_data.get('text', ''), 
                'translated': review_data.get('translated_response_text', ''),  
                'user_name': review_data.get('user_name', ''),  
                'user_reviews': review_data.get('user_reviews', 0),  
                'cat': review_data.get('other_ratings', ''),  
                'rating': review_data.get('rating', 0), 
            }
            
            if results_entry['reviews'] != '':
                results.append(results_entry)
        if results:
            writeFile(results, output_file)

if __name__ == "__main__":

    df = pd.read_csv(r"\\Admin-pc\data\2024\Oct\Output\Main\18_10_2024_main_hour_(success).csv", on_bad_lines='skip', engine='python')  
    base_dir = r"D:\DATA\2024\Oct\Output\Reviews"

    success_path = os.path.join(base_dir, "21_10_reviews_api.csv")
    # review_df = pd.read_csv(success_path,  on_bad_lines='skip', engine='python')
    # df = df[~df['code'].isin(review_df['code'])]
    header = 'code,reviews,translated,user_name,user_reviews,cat,rating\n'
    prepare_file(success_path, header)

    with ProcessPoolExecutor(max_workers=60) as executor:
        futures = [
            executor.submit(
                scrape_reviews_for_url, 
                row, 
                success_path
            ) 
